...
# dirs above. Exact files only, never a directory: site/_data/ also holds
# site.json (brand, menu, legal, company data), which IS config and must stay
# system-tier.
#
# biglobster/site/_data/langmap.json is the EN<->ES permalink pair table. It is
# publication data DERIVED from the post being published, not template or
# runtime config: every translation PR must add one entry in each direction or
# the new post ships with no hreflang. Leaving it system-tier meant EVERY
# translation the Translation Engineer opened was routed to the strong reviewer
# by that one file — biglobster#550 (2026-09-20) was 3 files of pure prose and
# still classified system, so it went to the reasoning judge and timed the gate
# out at 420s. The blast radius of a wrong entry is a bad hreflang link: an SEO
# regression, recoverable by the next build, not a runtime or secret risk.
#
# blog.html, sitemap.xml and feed.xml are deliberately NOT here: they are
# GENERATED by the Eleventy build from the posts collection, so a PR touching
# them is either stale-workflow noise or a template change — both system.
_REPO_EXTRA_CONTENT_FILES: dict = {
    "br41s/biglobster": (
