...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            # Fork: backoff + keyless provider fallback live INSIDE this one
            # tool call (tools/web_search_resilience.py). Retrying between
            # tool calls is what lost the 2026-09-16 Content Updater run: five
            # bare 503s in ~2s tripped the agent loop's
            # same_tool_failure_halt guardrail, which was correct — it cannot
            # tell a retry from a loop. Upstream's one-shot keyless-ring rescue
            # below is the last resort after the resilience layer gives up.
            ...
            def _paid_search() -> tuple[dict, bool]:
                # Returns (response, uncacheable). A response served by any
                # backend other than the chosen one (resilience fallback or
                # keyless rescue) must never be cached — see the store below.
                _fetch_limit = _bucket_limit(limit)
                _rescued = False
                try:
                    _outcome = search_with_resilience(
                        provider,
                        query,
                        _fetch_limit,
                    ...
                    _resp = _outcome.response
                    debug_call_data["providers_tried"] = [
                        r.as_dict() for r in _outcome.trail
                    ]
                    debug_call_data["total_wait_s"] = round(_outcome.total_wait_s, 2)
                    if _outcome.succeeded and _outcome.fell_back:
                        # Only a DEGRADED success annotates the payload. The
                        # normal success shape is left byte-for-byte as
                        # documented in agent/web_search_provider.py.
                        #
                        # Key names here deliberately avoid the literals
                        # '"error"' and '"failed"' that
                        # agent.tool_guardrails.classify_tool_failure scans
                        # for, so a degraded-but-successful search is not
                        # mislabelled a failure and does not burn guardrail
                        # budget.
                        _rescued = True
                        primary_trail = _outcome.trail[0]
                        _resp["search_provider"] = _outcome.provider_used
                        _resp["degraded"] = {
                            ...
                            "note": (
                                f"{primary_trail.provider} was unreachable; these "
                                f"results came from {_outcome.provider_used} "
                                "instead. Result quality and ranking may differ "
                                "from the usual backend."
                            ),
                        }
                        ...
                        # backend again.
                        _rescued = True
                        _rescue = _rescue_search(
                            provider.name,
                            str(_resp.get("error", "")),
                            query,
                            _fetch_limit,
                        )
                        if not _rescue.get("success"):
                            # Keep the resilience layer's typed outage signal
                            # (error_kind / per-provider trail) when the ring
                            # could not help either.
                            for _k in ("error_kind", "providers_tried"):
                                if _k in _resp:
                                    _rescue[_k] = _resp[_k]
                        _resp = _rescue
            ...
            if response_data is None:
                with _search_memo.flight_lock(provider.name, query, limit):
                    ...
                    if response_data is None:
                        ...
                        # Never cache a rescue- or fallback-served response:
                        # it came from another vendor, not the chosen backend,
