...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    if task_id and _session_isolation_enabled():
        key = _resolve_container_alias(task_id)
        # A child aliased to a missing parent lands on the literal "default";
        # keep that on the profile-scoped shared key (fork, see below).
        return _shared_default_container_key() if key == "default" else key
    ...
    if _docker_persistent_profile_scoped():
        ...
        if shared:
            return f"shared:{shared}"
    # Fork: the collapsed key stops at the profile edge -- see
    # _shared_container_profile_suffix(). Cron binds an empty session key, so
    # every cron job lands here regardless of profile.
    return _shared_default_container_key()


def _shared_default_container_key() -> str:
    """The collapsed shared container key for the active profile (fork)."""
    return "default" + _shared_container_profile_suffix()


def _is_shared_default_container_key(key: Optional[str]) -> bool:
    """True for the shared top-level key, profile-scoped or legacy literal."""
    return key == "default" or key == _shared_default_container_key()
...
def _resolve_task_host_cwd(config: Dict[str, Any], task_id: Optional[str]) -> Optional[str]:
    ...
    * Per-session isolation mode (docker + ``container_persistent: false``):
    ...
    if _is_shared_default_container_key(_resolve_container_task_id(task_id)):
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        session_scoped = (
            ...
            and not _is_shared_default_container_key(task_id)
