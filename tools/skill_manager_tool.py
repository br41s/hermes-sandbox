...
class _BackgroundReviewReadMarks:
    ...
    shown (fork). The digest (rather than a bare path set) is what lets a write
    invalidate an earlier read: see _background_review_read_is_current. The
    shared mutable object (upstream) is what lets a mark made in one copied
    tool context be seen by a write in another.
    """
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    marks.add(_resolved_str(path), _skill_file_digest(path))
...
def _background_review_read_is_current(path: Path) -> bool:
    ...
    if marks is None:
        return False
    recorded = marks.digest_of(_resolved_str(path))
    if recorded is None:
        return False
    return recorded == _skill_file_digest(path)
