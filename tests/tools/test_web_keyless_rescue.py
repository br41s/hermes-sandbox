...
def _keyed_keenable_env(monkeypatch):
    ...
    # Fork: tools/web_search_resilience.py retries the chosen backend and
    # tries a keyless fallback BEFORE upstream's rescue runs. These tests are
    # about the rescue, so give the chosen backend one attempt and no fork
    # fallback (the fork layer has its own tests in test_web_search_resilience).
    import tools.web_search_resilience as _resilience

    _orig = _resilience.search_with_resilience
    monkeypatch.setattr(
        _resilience,
        "search_with_resilience",
        lambda primary, query, limit, **kw: _orig(
            primary, query, limit, **{**kw, "fallbacks": (), "max_attempts": 1}
        ),
    )
    yield
