...
def _finish_trace(task_key: str, *, output: Any = None) -> None:
    ...
    try:
        ...
        if final_output is not None:
            # Root observation I/O only: trace-level IO (set_trace_io /
            # update_trace) is deprecated in langfuse v4 and deliberately not
            # called — v4 has no update_trace at all. Never let update() keep
            # end() from running, otherwise generations/tools export without
            # a CHAIN root. The root context is exited in ``finally`` below.
            try:
