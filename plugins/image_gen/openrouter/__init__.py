...
# This is the fallback behind upstream's _select_surface (curated
# _IMAGE_API_MODELS + live /images/models catalog): it only fires for a model
# neither of those routed to the Image API.
_IMAGES_API_REDIRECT = "cannot be used with the chat/completions endpoint"
_REDIRECTED_TO_IMAGES_API: set = set()
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _post_image_request(self, base_url, headers, model_id, content, or_aspect):
        ...
        if model_id in _REDIRECTED_TO_IMAGES_API:
        ...
        for modalities in _modality_attempts(model_id):
            ...
            try:
                ...
                if _is_images_api_redirect(exc):
                    ...
                    _REDIRECTED_TO_IMAGES_API.add(model_id)
                    logger.info(
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        for i, model_id in enumerate(model_chain):
            ...
            # Chat-completions surface. _post_image_request negotiates the
            # modality list and, if OpenRouter answers that the model lives on
            # the Image API after all, follows it there (fork: learned per
            # model, so an id missing from the curated table and the live
            # catalog still works).
            try:
