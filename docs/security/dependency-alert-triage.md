...
| `hermes-agent` (us) | Two VulDB advisories against our own published package. CVE-2026-10224 targets `gateway/platforms/feishu.py` — **that file does not exist in this fork**. CVE-2026-10221 states "up to 0.12.0"; we ship 0.19.0, and OSV records no fixed version, so *no upgrade can ever clear it*. These can only be closed by decision. |
