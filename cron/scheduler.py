...
# cron/fork_ext/diagnostics.py.
from cron.fork_ext.diagnostics import (  # noqa: E402
...
def _env_home_target_chat_id(platform_name: str, profile: Optional[str] = None) -> str:
    ...
    Fork: a job's own profile routing env wins first — read off its
    .env on disk (_read_profile_env_value), because delivery runs after
    the job's profile scope has been reset.
...
def _get_home_target_chat_id(platform_name: str, profile: Optional[str] = None) -> str:
    ...
    Resolution order: the job's own ``profile`` routing env (fork) → platform
    env var (legacy mirror, kept first so an operator override keeps winning)
    → legacy env var name → the canonical ``home_channel`` block persisted in
    config.yaml by ``/sethome``.
    """
...
# Kickoff ping (fork): see cron/fork_ext/kickoff.py. _deliver_result below is
# upstream's, unchanged.
from cron.fork_ext.kickoff import (  # noqa: E402
    send_kickoff_ping as _send_kickoff_ping,
    send_to_targets as _send_to_targets,
)
...
# Per-run isolated checkouts (fork): see cron/fork_ext/isolated_checkout.py.
from cron.fork_ext.isolated_checkout import (  # noqa: E402
...
def _run_job_impl(
    ...
    *,
    ...
    # Fork: run the agent in an ephemeral clone of a git workdir, so the
    # task-scoped cwd below points at the clone; fail closed if it cannot be
    # provisioned. Cleaned up in the finally at the end of this function. See
    # cron/fork_ext/isolated_checkout.py.
    ...
    try:
        ...
        try:
            ...
            if _cron_inactivity_limit is None and not _is_oneshot and cancel_event is None:
                ...
                while True:
                    ...
                    if _inactivity_timeout:
                        # Fork: dump the stuck agent's stacks and count the
                        # thread we are about to abandon (it is NOT killed —
                        # see cron/fork_ext/diagnostics.py).
            ...
            _watch_stop.set()
            # NOTE: not a kill — a timed-out agent thread survives this; see
            # abandoned_agent_threads() in cron/fork_ext/diagnostics.py.
            _cron_pool.shutdown(wait=False, cancel_futures=True)
...
"""
        ...
        # Fork: remove this run's ephemeral isolated checkout, if one was provisioned.
...
def _run_one_job_body(
    ...
    *,
    ...
    try:
        ...
        # before the (possibly slow) final result arrives. Non-fatal: a ping
        # failure must never block the actual job run. Lives in the shared
        # body (not tick's closure) so BOTH the built-in ticker and an external
        # provider's fire_due emit it — the closure-local version was lost when
        # tick became a thin wrapper around run_one_job.
        try:
            _send_kickoff_ping(job, adapters=adapters, loop=loop)
        except Exception as ke:
            logger.warning("Job '%s': kickoff ping raised (non-fatal): %s",
                           job.get("id", "?"), ke)
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # (fork: except profile/workdir jobs, which _fork_submit_sequential
        # below moves onto the fork's single-thread lane).
        ...
        def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor):
            ...
            try:
                # Fork: adapters is only ever populated by the gateway's own
                # in-process ticker (InProcessCronScheduler.start,
                # scheduler_provider.py) — a standalone `hermes cron tick`
                # always calls tick() with adapters=None. That's the one
                # reliable signal distinguishing a warm gateway tick from a
                # one-shot CLI process (confirmed investigating an unattributed
                # 2026-08-28 run). Recorded here instead of guessed from logs.
