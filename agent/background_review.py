...
_SKILL_REVIEW_PROMPT = (
    ...
    "     BEFORE writing a new support file, call skill_view(name) and "
    "read the list of files the skill already has. If one of them "
    "already covers this subject, PATCH IT — do not add a sibling "
    "under a slightly different name. Nothing downstream ever merges "
    "them: an unchecked references/ directory accretes a dozen files "
    "on one topic and future agents read whichever they hit first. A "
    "new file is justified only by a genuinely new subject.\n"
    ...
    "FIX THE ROOT, NOT THE LEAF (important): SKILL.md is what gets "
    "loaded into context; references/ is detail that may never be "
    "opened. So when this session proved a documented procedure WRONG "
    "— it is blocked, it errors, the flag does not exist, the path is "
    "refused — correcting a reference file is not enough and can be "
    "worse than nothing, because the wrong instruction stays in the "
    "part that is actually read. Call skill_view(name), find every "
    "place SKILL.md teaches the superseded procedure, and patch those "
    "too, in this same pass. A reference file is detail, never errata "
    "for its own SKILL.md.\n\n"
    "If a patch to SKILL.md is refused because the skill is over its "
    "size budget, say so explicitly in your reply and name the skill. "
    "That skill is frozen until the curator splits it, and a silent "
    "fallback to writing a reference file instead is exactly how the "
    "stale instruction survives.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "FIX THE ROOT, NOT THE LEAF: SKILL.md is what gets loaded; "
    "references/ is detail that may never be opened. When this session "
    "proved a documented procedure WRONG (blocked, errors, flag does "
    "not exist, path refused), patching a reference file is not "
    "enough — skill_view the skill, find every place SKILL.md still "
    "teaches the superseded procedure, and fix those too in the same "
    "pass. If a SKILL.md patch is refused for exceeding the size "
    "budget, say so and name the skill rather than quietly writing a "
    "reference file instead.\n\n"
    "Before adding a new support file, check the skill's existing "
    "files and patch the one that already covers the subject. Nothing "
    "downstream merges near-duplicate references.\n\n"
