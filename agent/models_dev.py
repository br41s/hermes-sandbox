...
Data resolution order:
  ...
  4. Bundled snapshot (``models_dev_snapshot.json``, ships with the package)
     — only after a network attempt found no memory cache, no disk cache
     and no reachable models.dev (``allow_network=False`` callers still get
     ``{}``: they never attempted the network, so "offline" is unknown)
...
identity rather than a substitute for the catalog. It is never stored as
the in-memory cache, so it can never shadow (or be ETag-"confirmed" as) a
real registry. It exists because a cold install with no network otherwise
resolves no provider at all, and ``hermes_cli.providers.get_provider()``
sources a provider's ``api_key_env_vars`` from here: without it,
``OPENROUTER_API_KEY`` is invisible to ``is_provider_explicitly_configured()``
and OpenRouter vanishes from the model picker despite being configured.
Regenerate it with ``scripts/generate_models_dev_snapshot.py``.
...
def _bundled_snapshot_floor() -> Dict[str, Any]:
    """The bundled snapshot as a last-resort answer, never cached in memory.

    A top-level copy, not an alias: the snapshot is memoised for the process,
    so handing callers the same mapping would let one stray
    ``data[provider] = ...`` outlive the call. Entries carry no ``models``
    key, so every model-level query still degrades to "unknown" — this widens
    what we know about providers, never about models.
    """
    snapshot = _load_bundled_snapshot()
    if snapshot:
        logger.debug(
            "Serving bundled models.dev snapshot (%d providers, no model data)",
            len(snapshot),
        )
...
def fetch_models_dev(
    ...
    Cache hierarchy (when ``force_refresh=False``):
      ...
      6. Nothing at all (no memory cache, no disk cache, network failed or
         backing off) → return a copy of the package-bundled provider
         snapshot (provider metadata only, no models). It is not stored in
         memory, so the next call retries the real sources per the backoff.
    ...
    if not force_refresh and time.time() < _models_dev_retry_after:
        return _models_dev_cache or _bundled_snapshot_floor()
    ...
    with _models_dev_fetch_lock:
        ...
        # Stage 6: nothing at all — a cold install that has never reached
        # models.dev. Serve the bundled provider snapshot so provider
        # identity (crucially ``env``, the API-key env vars) still resolves.
        return _models_dev_cache or _bundled_snapshot_floor()
