...
# Fork-owned routes (/health, cron job copy, /api/delegate, rental webhook)
# live in hermes_cli/fork_ext/web.py so upstream merges of this file stay clean.
from hermes_cli.fork_ext.web import router as _fork_router  # noqa: E402

app.include_router(_fork_router)
...
def mount_spa(application: FastAPI):
    ...
    # Cache rendered index.html per (prefix, gated, index mtime).  The
    # injected values (_SESSION_TOKEN, auth_required, embedded-chat flag) are
    # fixed for the lifetime of the server process, so reading and rewriting
    # the HTML on every SPA route navigation is pure waste.  The mtime in the
    # key (one stat per request) keeps upstream's dynamic recheck working: a
    # vanished index 404s and a rebuilt one is picked up without a restart.
    ...
    def _serve_index(prefix: str = ""):
        ...
        try:
            _index_mtime = _index_path.stat().st_mtime_ns
            _cache_key = (prefix, gated, _index_mtime)
        ...
        # Drop renders of an older index.html so a rebuild can't grow this.
        for _stale_key in [k for k in _index_cache if k[2] != _index_mtime]:
            _index_cache.pop(_stale_key, None)
    ...
    async def serve_css(filename: str, request: Request):
        ...
        css = await asyncio.to_thread(
            lambda: css_path.read_text(encoding="utf-8")
        )
        if prefix:
