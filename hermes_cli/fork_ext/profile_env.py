...
def fal_client_for_current_key(fal_module: Any) -> Any:
    ...
    each profile on its own key. With no key in scope, the process's own key,
    or a stand-in module without ``SyncClient``, the module itself is
    returned: exactly the old call.
    """
    try:
