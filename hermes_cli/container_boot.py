    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            if entry.name in _GATEWAYLESS_PROFILES:
                ...
                # `multiplex_profiles` (upstream): when a single gateway
                # multiplexes every profile, per-profile slots must stay down
                # or they would double-poll the shared bot.
                should_start = not multiplex_profiles and _should_autostart(prior)
...
def _read_prior_state(profile_dir: Path) -> _PriorState:
    ...
    try:
        ...
        # Upstream's ``desired_state`` wins when present: newer lifecycle
        # commands persist the operator's durable intent separately, so a
        # transient observed state can't be mistaken for it. Falling back to
        # ``gateway_state``, normalise the transient running-ish states
        # (e.g. ``draining`` after a hard kill) or the slot stays DOWN forever.
        _desired = data.get("desired_state")
        if _desired is not None:
            _effective = _desired
        else:
            _effective = data.get("gateway_state")
            if _effective in _TRANSIENT_RUNNING_STATES:
                _effective = "running"
