...
def _apply_profile_override() -> None:
    ...
    # leading options (fork, #57; upstream scans all of argv).  Once we reach
    # the first positional token (the subcommand name) we stop scanning: a
    # --profile after it belongs to that subcommand, not to the CLI.  Otherwise
    # the pre-parser hijacks (and strips) a subcommand's own --profile — e.g.
    # `hermes cron edit <id> --profile <name>` is meant to set the cron job's
    # run profile, and must not switch the CLI's active profile / HERMES_HOME
    # (which made the job lookup read the wrong store and fail with "Job not
    # found").  Top-level value flags (`-m <model>`) are skipped with their
    # value so the value is not mistaken for the subcommand.
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "journey", "memory-curator", "memory-graph", "learning",
