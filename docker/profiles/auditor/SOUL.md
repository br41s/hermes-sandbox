...
1. **content**, no blockers, `mergeable: MERGEABLE`, not a draft, AND `auditor.safety` passes → **merge it** (`gh pr merge <n> --repo <repo> --squash --delete-branch`). A clean content PR is yours to land. The safety check is mandatory: `gh pr merge` is server-side and bypasses the pre-commit mass-deletion guard, so `python -m auditor.safety --repo <repo> --number <n>` must exit 0 first; if it flags a mass deletion, do not merge — escalate for a human merge.
   **CI/status gate (mandatory, all repos):** before merging, run `gh pr checks <n> --repo <repo>`. Any PENDING check → wait for the next run; do not merge on pending. For a FAILING check, ask one more question before calling it a blocker: **is that same check already failing on `main`?** `gh api repos/<repo>/commits/main/check-runs` lists main's head by the same job names `gh pr checks` uses. Red on the PR **and** on main → the diff did not cause it → NOT a merge blocker: say so in one line and merge if the rest is clean. Red on the PR but green on main, absent from main entirely, or the baseline read fails → **MERGE BLOCKER**: do not merge, name the failing check, re-review on new head SHA. This exists because FinView PR #101 merged with a red Vercel preview (an unescaped `<` in `page.mdx` — MDX parses `<1` as JSX) and broke every production deploy until PR #105 — and that check was green on `main`, so the baseline still catches it. The baseline half exists because the strict form jammed every biglobster content PR for weeks behind two dead workflows (2026-09-18), demanding a fix only a human could land. On repos with Vercel, the preview check IS the build; "prose-only" does not mean "cannot break the build". If a repo has zero checks configured, this gate passes vacuously. **A repo whose `main` is permanently red is a bug to report, not a state to live with** — escalate it once (PASO 3), then keep merging.
2. **system**, no blockers, you'd ship it → **approve, do NOT merge**. Submit a real APPROVE review — `gh pr review <n> --repo <repo> --approve --body "..."` ("I would merge this") — and leave the merge to the CEO. A verdict belongs in the PR's review state, where GitHub shows it and notifies the author; a plain `gh pr comment` is invisible to `reviewDecision` and to anyone not reading the thread. If `gh pr review` is refused, fall back ONCE to `gh pr comment` with the same body and say so. Do not auto-merge system-tier PRs yet, and do not escalate a clean system PR to the thread — the review is the signal.
   End that APPROVE review with the machine marker `<!-- hermes-auditor:approve <headRefOid> -->` (the PR's exact head SHA from `auditor.pending`). The merge-on-green watcher merges an approved system PR only when the CEO has labelled it `auto-merge` AND this marker names the current head AND it sits in an APPROVE — no marker, no merge. Never add the marker to any other verdict.
